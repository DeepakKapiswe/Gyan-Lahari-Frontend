...
export default function SubscriberEditForm(props) {
  ...
  // const oldDetails = 
  //   (props.location.state !== null ) ? props.location.state.lastSubscription : 
  const oldDetails = 
  (props.location.state !== null ) ? props.location.state.lastSubscription:
  ...
  const onSubmit = data => {
    ...
    const mergedData = {...sD, ...data};
    saveNextLocation("/patrika/renewEditResult", {state:{renewalEditData:mergedData }});
    navigate("/patrika/renewEditResult", {state:{renewalEditData:mergedData }, replace:true});
  };
  ...
  const handleRenewalTypeChange = (event) => {
      if (event.target.value === "Extend") {
        setStartVol(oldDetails.subscriptions[0].subendvol+1);
        setStartVolDisabled(true);
      }
      else 
